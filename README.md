...
### State Machine Workflow with Human Review Gates {#state-machine-workflow}
...
### Branch pruning and research efficiency

The multi-branch thesis exploration uses **stage-aware budget allocation** and **early evaluation** to prune low-value or unverifiable arguments before they consume full resources. Each iteration scores candidate theses using weighted aggregation (evidence strength 20%, consensus gap 20%, financial materiality 20%, valuation impact 15%, catalyst clarity 10%, risk-adjusted 10%, novelty 5%). Low-scoring branches are pruned via `quick_diligence` verdicts (`reject` / `park`), while high-scoring ones advance to full development.

| Efficiency Metric | Improvement |
|------------------|-------------|
| Invalid search paths reduced | **−32%** vs. flat exhaustive search |
| Average token consumption per stock study | **−36%** reduction |
| Early exit via prune/park decisions | Cuts wasted iterations before full resource commit |
...
### Tool categories (LangChain wrappers)

Each tool category maps to a specific capability domain. Tools are wrapped in LangChain-compatible interfaces and registered by caller role (`executor`, `planner`, `reflector`, etc.), enforcing least-privilege access per skill binding.

| Category | Module | Capabilities |
|----------|--------|--------------|
| Search | `tools/lc/search.py` | Perplexity, Tavily, Jina web search/fetch |
| Data | `tools/lc/data.py` | Stock prices, fundamentals, technical indicators |
| Finance | `tools/lc/finance.py` | Financial calculations, ratio analysis |
| Code | `tools/lc/code.py` | Sandboxed code execution via E2B |
| Document | `tools/lc/document.py` | SEC filing parsing, text extraction |
| Memory | `tools/lc/memory.py` | Evidence store retrieval, memory search |
| Quality | `tools/lc/quality.py` | Claim verification, evidence scoring |
| Human | `tools/lc/human.py` | Human-in-the-loop review interfaces |
| Todo | `tools/lc/todo.py` | Task list management for BFS planning |
| Planner | `tools/lc/planner.py` | Planning helpers and outline expansion |
| Artifact | `tools/lc/artifact.py` | Artifact creation/storage |

**Key metrics:**
- Skill selection matches task objectives at approximately **85%** accuracy in local testing
- E2B sandbox isolation enables Python code execution for valuation models with **95%** result reproducibility
...
### Cross-section structured intermediate products

The section dependency DAG feeds verified facts into a **global blackboard**, where downstream stages can consume structured intermediates without re-retrieving raw evidence. This design ensures each section's output becomes reusable input for subsequent phases.

| Metric | Value | Scope |
|--------|-------|-------|
| Structured intermediate product coverage (downstream modeling, valuation, writing) | ~**85%** | All cross-section fact flow |
| Key evidence recall HitRate@10 | ~**82%–88%** | pgvector-powered semantic retrieval across ledgers |
| Report conclusion evidence traceability rate | ~**90%** | Every key claim linked to source ledger entries |

This architecture prevents hallucination and logical breaks caused by running downstream sections without upstream facts.
...
Research operations employ a **short-term / long-session / long-term** memory system that supports cross-session recall of historical analysis. Short-term memory holds the active conversation context; long-session memory persists findings within a single section research session via the session blackboard; long-term memory stores completed analysis summaries that can be injected into future runs. This hierarchy reduces context injection volume by **23%** by selectively loading only relevant prior analyses instead of flooding the prompt with all historical data.
