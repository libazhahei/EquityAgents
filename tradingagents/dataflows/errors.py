...
these (or a thin vendor-named subclass) and needs no new ``except`` clause.
    ...
    ├── VendorSubscriptionError    plan/subscription does not include endpoint
...
class VendorSubscriptionError(VendorError):
    """A vendor rejected the request because the current plan lacks the endpoint."""
